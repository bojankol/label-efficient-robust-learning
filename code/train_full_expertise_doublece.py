...
def main():
    ...
    if args.uniform_labeler_weights == 1:
        filepath = f"logs/full/{date.today().strftime('%d-%m-%Y')}_{args.lmd}_{args.task}_{args.percent_gt}_uniform"
    else:
        filepath = f"logs/full/{date.today().strftime('%d-%m-%Y')}_{args.lmd}_{args.task}_{args.percent_gt}"
    ...
    for iteration in range(args.iterations):
        ...
        for samples, test_samples, annotations, test_annotations, ground_truth, test_ground_truth in folds:
            ...
            if args.v_init_type == "random":
                ...
                v = init_v_uniform_matrix(annotations, device)
            ...
            while active_annotators > goal_annotators:
                ...
                v, loss, accuracy = train(epoch, optim, model, samples, samples_chosen, indices_chosen, annotations, ground_truth, ground_truth_chosen, v, psi, lmd, timestep, args.interval_length, device, args.uniform_labeler_weights, p=p, )
                print(v.shape)
                active_annotators = get_active_annotators(v)
                ...
                if epoch % args.interval_length == 0:
                    ...
                    test_loss, acc, f1_score, precision, recall, test_predictions, test_auc_score, outputs = test(model, test_samples, test_annotations, test_ground_truth, v_pred, psi, lmd)
                    output_list.append(outputs.cpu())
                    test_loss_hist.append(test_loss)
                    test_acc_hist.append(acc)

                    print(f'train epoch: [{"%03d" % epoch}], acc: [{acc}%], active_annotators: [{active_annotators}]')
            ...
            # test_predictions =
            save_logs_(train_loss_hist, test_loss_hist, v_hist, train_acc_hist, test_acc_hist, test_ground_truth,
